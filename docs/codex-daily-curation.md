...
X Latest 若忽略 `since` / `until` 日期条件，记录历史补查为 `partial`，继续补查旧时段，但不因此否定已读到的当日或尚未收录原帖。GitHub 与 X 都已实际返回并核实一手来源时，可按下述质量标准发布这些新条目；不得把未覆盖的旧时段写成“没有新案例”。登录墙、加载占位或没有真实搜索结果回读仍算来源失败，停止发布。
