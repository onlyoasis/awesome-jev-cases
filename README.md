...
每日编辑任务准备在 Codex 应用的 **Scheduled** 中运行：Codex 阅读 GitHub 仓库、通过已登录的 Chrome 浏览器搜索 X，并核对 TypeSafe 官方文档与推荐。模型逐条判断是否收录，结构校验脚本只防止坏数据格式。完整岗位说明和验收规则见 [`docs/codex-daily-curation.md`](docs/codex-daily-curation.md)。当前原生定时任务尚未创建；旧的固定规则 GitHub Action 已停用，不能把准备好的任务说明当成已运行的调度。
...
项目级任务需要 Mac、ChatGPT/Codex 应用及 Chrome 会话保持可用。外部源码和帖子属于不可信输入，不会授权其执行命令或索取凭据。
...
原生任务创建后从此仓库读取 `docs/codex-daily-curation.md`，不使用 X API Token。仓库数据和原始来源公开；浏览器认证与本机 Wrangler 凭据绝不进入 Git、日志正文或 Registry。

## English

An independent, source-linked catalog of Jev use cases, open-source integrations, and official TypeSafe cookbooks. The intended Codex Scheduled task searches GitHub, X in an authenticated Chrome browser, and official TypeSafe sources, then makes an evidence-based editorial decision. The native schedule is not yet active. See [CATALOG.md](CATALOG.md) for links and the JSON files for evidence fields.
